...
def inspector() -> DeveloperInspectorController:
    controller = DeveloperInspectorController(_app(), "v0.1.4")
    yield controller
    controller.close()
    _app().processEvents()
...
def test_f12_is_the_only_enable_path_and_each_controller_starts_off(
    ...
    replacement = DeveloperInspectorController(app, "v0.1.4")
    try:
...
def test_request_copy_uses_only_fixed_metadata_and_leaves_user_fields_blank(
    ...
    assert build_static_request_text(metadata, "v0.1.4") == expected
